#ifndef JETSON_SEGMENT_YOLOV8_SEG_CPP
#define JETSON_SEGMENT_YOLOV8_SEG_CPP
...
void YOLOv8_seg::postprocess(std::vector<seg::Object>& objs,
	float score_thres,
	float iou_thres,
	int topk,
	int seg_channels,
...
void YOLOv8_seg::draw_contour(const cv::Mat& image,
	cv::Mat& res, const std::vector<seg::Object>& objs)
{
	cv::Scalar mask_color(255, 255, 255);
	res = image.clone();
	...
	for (auto& obj : objs)
	{
		...
		mask(obj.rect).setTo(mask_color, obj.boxMask);
		mask(obj.rect) = obj.boxMask;

    	std::vector<std::vector<cv::Point>> contours;
    	cv::findContours(mask, contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
		
		// Draw contours on the output image
    	cv::drawContours(res, contours, -1, cv::Scalar(255, 0, 0), 8);
	}
}

#endif //JETSON_SEGMENT_YOLOV8_SEG_CPP
